...
def build_table(acc: RevenueAccumulator, account: str, year: int, month: int,
                usd_to_rmb: float) -> PrettyTable:
    ...
    table = PrettyTable(["Product", "Revenue ($)", "Adjusted Fee ($)", "Adjusted Revenue ($)", "Email", "Rate (%)", "Net Payout ($)", "Net Payout (¥)"])
    table.title = (
    ...
    for prod, rev in acc.entries():
        ...
        table.add_row([
            ...
            rate_display,
            f"{net_payout / 100:.2f}",
            f"{net_payout * usd_to_rmb / 100:.2f}",
        ])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--usd-to-rmb", type=float, default=6.79,
                        help="USD→RMB exchange rate used for the Net Payout (¥) column (default: 6.79)")
...
def main() -> None:
    ...
    with StripeCache() as cache:
        ...
        table = build_table(acc, account, args.year, args.month, args.usd_to_rmb)
        print(table)
