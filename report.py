...
def build_table(acc: RevenueAccumulator, year: int, month: int) -> PrettyTable:
    ...
    table.title = (
        f"Stripe Revenue Report for {year}-{month:02d}"
...
def main() -> None:
    ...
    with StripeCache() as cache:
        ...
        table = build_table(acc, args.year, args.month)
        print(table)
        save_outputs(acc, table, args.year, args.month)
